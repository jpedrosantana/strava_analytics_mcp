...
def _patch_stdio_empty_lines() -> None:
    """Monkey-patch mcp stdio transport to skip empty lines before JSON parsing.

    MCP fastmcp/server.py does `from mcp.server.stdio import stdio_server`, so we
    must patch the local reference there — patching mcp.server.stdio alone has no effect.
    """
    import sys
    ...
    @asynccontextmanager
    async def _fixed_stdio_server(stdin=None, stdout=None):
        if not stdin:
    ...
    stdio_mod.stdio_server = _fixed_stdio_server
    fastmcp_server_mod.stdio_server = _fixed_stdio_server
